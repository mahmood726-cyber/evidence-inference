#!/usr/bin/env python3
# Write MAFI v2.2.0 R package code

mafi_code = r'''#' MAFI: Multi-Signal Aggregate Funnel Index v2.2.0
#'
#' A publication bias detection tool that combines multiple statistical signals
#' into a calibrated probability score (0-100). Version 2.2.0 addresses signal
#' correlation and heterogeneity handling based on methodological review.
#'
#' @name MAFI-package
#' @docType package
...
# ============================================================
# MAFI v2.2.0 - METHODOLOGICAL REVISION
# ============================================================
# KEY CHANGES from v2.1.1:
# 1. Signal correlation: Clustered correlated signals, use max within cluster
# 2. I2 handling: REVERSED - high I2 now INCREASES suspicion (not decreases)
# 3. Reliability: Changed from multiplicative penalty to CI widening only
# 4. Validation: Added theta=0 (null effect) for specificity testing
# 5. Calibration: Added calibration slope metric
# 6. Clusters: Asymmetry (Egger/PET/Begg/precision), Selection, TES, T&F
# ============================================================

# ============================================================
# SIGNAL CLUSTER DOCUMENTATION
# ============================================================
# Signals are grouped by what they measure to avoid double-counting:
#
...
#   -> Combined: Take MAXIMUM signal within cluster (most conservative)
#
# CLUSTER 2 - Selection Models (partially correlated):
#   - sel_lrt_pval: 3-parameter selection model (p<0.025)
#   - sel_lrt_pval_05: 3PSM with p<0.05 cutoff
#   - sel_beta_pval: Beta selection model
#   -> Combined: Bonferroni-adjusted minimum p-value
#
# CLUSTER 3 - Test of Excess Significance (independent):
#   - excess_sig_pval: Compares observed vs expected significant studies
#   -> Used directly (independent of asymmetry)
#
# CLUSTER 4 - Trim-and-Fill (semi-independent):
#   - taf_k0_ratio: Proportion of imputed studies
#   -> Used directly (mechanistically different from regression)
#
# Final weights on clusters (not individual signals):
#   - asymmetry_cluster: 0.40 (primary evidence type)
#   - selection_cluster: 0.25 (direct modeling of selection)
#   - excess_sig: 0.20 (independent, high specificity)
#   - trim_fill: 0.15 (imputation-based, lower reliability)
# ============================================================

.mafi_cluster_weights <- list(
  asymmetry = 0.40,
  selection = 0.25,
  excess_sig = 0.20,
  trim_fill = 0.15
...
# - All p-value thresholds set to 0.10 (liberal for screening)
# - Logistic k=4: Smooth transition around threshold
#
# I2 handling (REVISED in v2.2.0):
# - i2_boost_max (0.15): High I2 INCREASES score by up to 15%
#   Rationale: High heterogeneity often accompanies publication bias
#   (small studies with different results suppressed)
#
# Reliability (REVISED in v2.2.0):
# - n_reliability_threshold (10): Point estimate unchanged for n>=10
# - For n<10: Only CI is widened, not point estimate penalized
#
# Classification thresholds (25, 40, 55, 70):
# - Calibrated against simulation with known bias levels
# ============================================================
...
mafi_score <- function(yi, vi, signals = NULL, bootstrap = FALSE, n_boot = 200) {
  ...
  params <- .mafi_params
  k <- params$logistic_k
  cw <- .mafi_cluster_weights

  compute_score <- function(sig) {
    # ============================================================
    # CLUSTER 1: Funnel Asymmetry (correlated signals)
    # Take MAXIMUM signal to avoid double-counting
    # ============================================================
    asymmetry_signals <- c()
    ...
    if (!is.na(sig$begg_pval)) {
      asymmetry_signals <- c(asymmetry_signals, 1 / (1 + exp(k * (sig$begg_pval - params$begg_threshold))))
    }

    if (!is.na(sig$precision_effect_cor)) {
      asymmetry_signals <- c(asymmetry_signals, pmin(1, pmax(0, -sig$precision_effect_cor / params$precision_cor_scale)))
    }
    ...
    if (!is.na(sig$small_study_effect) && !is.na(sig$small_study_direction)) {
      ...
      if (sig$small_study_direction == "inflated") {
        asymmetry_signals <- c(asymmetry_signals, effect_magnitude)
      } else if (sig$small_study_direction == "deflated") {
        asymmetry_signals <- c(asymmetry_signals, params$deflation_weight * effect_magnitude)
      }
    }

    # Use MAX within asymmetry cluster (most suspicious signal wins)
    cluster_asymmetry <- if (length(asymmetry_signals) > 0) max(asymmetry_signals) else 0.5

    # ============================================================
    # CLUSTER 2: Selection Models (Bonferroni-adjusted)
    # ============================================================
    if (!is.na(sig$sel_min_pval_adjusted)) {
      cluster_selection <- 1 / (1 + exp(k * (sig$sel_min_pval_adjusted - params$sel_threshold)))
    } else if (!is.na(sig$sel_lrt_pval)) {
      cluster_selection <- 1 / (1 + exp(k * (sig$sel_lrt_pval - params$sel_threshold)))
    } else {
      cluster_selection <- 0.5
    }
    ...
    if (!is.na(sig$excess_sig_pval)) {
      cluster_excess <- 1 / (1 + exp(k * (sig$excess_sig_pval - params$excess_threshold)))
    } else {
      cluster_excess <- 0.5
    }

    # ============================================================
    # CLUSTER 4: Trim-and-Fill
    # ============================================================
    if (!is.na(sig$taf_k0_ratio)) {
      cluster_taf <- pmin(1, sig$taf_k0_ratio / params$taf_ratio_scale)
    } else {
      cluster_taf <- 0.5
    }

    # ============================================================
    # Combine clusters with weights
    # ============================================================
    raw_score <- cw$asymmetry * cluster_asymmetry +
                 cw$selection * cluster_selection +
                 cw$excess_sig * cluster_excess +
                 cw$trim_fill * cluster_taf

    # ============================================================
    # I2 ADJUSTMENT (REVISED v2.2.0)
    # High I2 INCREASES suspicion of bias (not decreases)
    # ============================================================
    i2_adj <- 1 + params$i2_boost_max * (sig$i_squared / 100)

    # ============================================================
    # RELIABILITY (REVISED v2.2.0)
    # No point estimate penalty - only affects CI width
    # Score is unpenalized; small n handled via classification confidence
    # ============================================================
    score <- 100 * raw_score * i2_adj
  ...
  # Widen CI for small n instead of penalizing point estimate
  ci_width_factor <- if (n < 10) 1.5 else if (n < 15) 1.2 else 1.0
  ci <- quantile(boot_scores, c(0.025, 0.975))
  ci_center <- mean(c(ci[1], ci[2]))
  ci_half_width <- (ci[2] - ci[1]) / 2 * ci_width_factor
  ci_lower <- max(0, ci_center - ci_half_width)
  ci_upper <- min(100, ci_center + ci_half_width)

  result <- list(score = main_score, ci_lower = round(ci_lower, 1), ci_upper = round(ci_upper, 1),
...
mafi <- function(yi, vi, sei = NULL, measure = "Effect Size", bootstrap = FALSE, n_boot = 200) {
  ...
  result <- list(score = score, score_ci = score_ci, boot_failure_rate = boot_failure_rate,
                 ...
                 signals = signals, correction_methods = correction$methods, measure = measure, version = "2.2.0")
  class(result) <- "mafi"
  return(result)
}
...
print.mafi <- function(x, ...) {
  cat("\n======================================================\n")
  cat("  MAFI: Multi-Signal Aggregate Funnel Index v2.2.0\n")
  cat("======================================================\n\n")
...
summary.mafi <- function(object, ...) {
  cat("\n======================================================\n")
  cat("  MAFI v2.2.0 DETAILED SUMMARY\n")
  cat("======================================================\n\n")
...
mafi_report <- function(x, format = "text") {
  ...
  if (format == "text") {
    sprintf("MAFI v2.2.0 score was %s (%s; k=%d, I^2=%.1f%%). Original estimate: %.3f, corrected: %.3f (%.1f%% change).",
            score_text, risk_text, x$n_studies, x$i_squared, x$original, x$corrected, abs(x$attenuation))
  } else if (format == "markdown") {
    sprintf("**MAFI v2.2.0:** %s (%s) | Original: %.3f | Corrected: %.3f", score_text, x$classification$class, x$original, x$corrected)
...
simulate_biased_ma <- function(k = 20, theta = 0.3, tau = 0.1, bias_strength = 0.5,
  ...
  } else if (bias_type == "precision") {
    se_thresh <- median(sei)
    ifelse(sei < se_thresh, 1, 1 - bias_strength * 0.7)
  } else if (bias_type == "time_lag") {
    ifelse(yi > 0, 1, 1 - bias_strength * 0.5)
  } else rep(1, n_gen)
...
validate_mafi <- function(n_sims = 100, k_range = c(10, 20, 50), bias_levels = c(0, 0.25, 0.5, 0.75, 1.0),
                          theta_range = c(0, 0.2, 0.5),
                          tau_range = c(0.05, 0.15),
                          bias_types = c("significance", "direction", "both"),
                          seed = 42, verbose = TRUE) {
  set.seed(seed)
  results <- data.frame()
  total <- length(k_range) * length(bias_levels) * length(theta_range) * length(tau_range) * length(bias_types) * n_sims
  count <- 0
      ...
      for (tau in tau_range) {
        for (bias_type in bias_types) {
          for (bias in bias_levels) {
            for (i in 1:n_sims) {
              count <- count + 1
              if (verbose && count %% 100 == 0) cat(sprintf("Progress: %d/%d (%.1f%%)\n", count, total, 100*count/total))
              dat <- tryCatch(simulate_biased_ma(k = k, theta = theta, tau = tau, bias_strength = bias, bias_type = bias_type), error = function(e) NULL)
              if (is.null(dat)) next
              mafi_result <- tryCatch(mafi(dat$yi, dat$vi), error = function(e) NULL)
              if (is.null(mafi_result)) next
              results <- rbind(results, data.frame(
                k = k, theta = theta, tau = tau, bias_type = bias_type,
                true_bias = bias, true_bias_binary = as.numeric(bias > 0.25),
                mafi_score = mafi_result$score,
                original_estimate = mafi_result$original,
                corrected_estimate = mafi_result$corrected,
                true_theta = dat$true_theta[1],
                naive_bias = dat$naive_estimate[1] - dat$true_theta[1],
                is_null = as.numeric(theta == 0)
              ))
            }
          }
        }
  ...
  # Sensitivity/Specificity at threshold 40
  threshold <- 40
  sensitivity <- mean(results$mafi_score[results$true_bias_binary == 1] >= threshold)
  specificity <- mean(results$mafi_score[results$true_bias_binary == 0] < threshold)

  # Calibration slope (regression of true bias on predicted score)
  cal_model <- lm(true_bias ~ I(mafi_score/100), data = results)
  calibration_slope <- coef(cal_model)[2]
  calibration_intercept <- coef(cal_model)[1]

  # Null effect specificity (type I error rate)
  null_results <- results[results$is_null == 1, ]
  if (nrow(null_results) > 0) {
    null_specificity <- mean(null_results$mafi_score < threshold)
    null_mean_score <- mean(null_results$mafi_score)
  } else {
    null_specificity <- NA
    null_mean_score <- NA
  }
  ...
  list(
    ...
    null_specificity = round(null_specificity, 3),
    null_mean_score = round(null_mean_score, 1),
    correction_effectiveness = round(correction_effectiveness, 3),
    n_sims_successful = nrow(results),
    summary = sprintf("MAFI v2.2.0 Validation: AUC=%.3f, RMSE=%.3f, Sens=%.1f%%, Spec=%.1f%%, CalSlope=%.2f, NullSpec=%.1f%%, Correction=%.1f%%",
                      auc, rmse, sensitivity*100, specificity*100, calibration_slope, null_specificity*100, correction_effectiveness * 100)
  )
}
'''

# Write the file
with open("C:/Users/user/MAFI/R/mafi.R", "w", encoding="utf-8", newline="\n") as f:
    f.write(mafi_code)

print("MAFI v2.2.0 written successfully!")
