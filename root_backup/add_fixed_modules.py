...
insert_marker = '</script></body></html>'
...
const ComparisonAdjustedFunnel={
    ...
    render(canvas,data){
        ...
        const allPoints=[];
        data.comparisons.forEach(c=>c.studies.forEach(s=>allPoints.push({x:s.centered,y:s.se,comp:c.comparison})));
        if(allPoints.length===0)return;
        const maxSE=Math.max(...allPoints.map(p=>p.y))*1.1;
        const maxX=Math.max(...allPoints.map(p=>Math.abs(p.x)))*1.2;
        const margin={top:40,right:60,bottom:50,left:60};
        const plotW=w-margin.left-margin.right;
        const plotH=h-margin.top-margin.bottom;
        const scaleX=x=>(x+maxX)/(2*maxX)*plotW+margin.left;
        const scaleY=y=>margin.top+(y/maxSE)*plotH;
        ctx.strokeStyle='#dee2e6';ctx.beginPath();ctx.moveTo(scaleX(0),margin.top);ctx.lineTo(scaleX(0),h-margin.bottom);ctx.stroke();
        ctx.beginPath();ctx.moveTo(scaleX(0),margin.top);ctx.lineTo(scaleX(-maxSE*1.96),h-margin.bottom);ctx.lineTo(scaleX(maxSE*1.96),h-margin.bottom);ctx.closePath();ctx.fillStyle='rgba(0,123,255,0.1)';ctx.fill();
        const colors=['#007bff','#28a745','#dc3545','#ffc107','#17a2b8'];
        const compList=[...new Set(allPoints.map(p=>p.comp))];
        allPoints.forEach(p=>{
            const colorIdx=compList.indexOf(p.comp)%colors.length;
            ctx.fillStyle=colors[colorIdx];
            ctx.beginPath();ctx.arc(scaleX(p.x),scaleY(p.y),5,0,Math.PI*2);ctx.fill();
        });
        ctx.fillStyle='#333';ctx.font='12px sans-serif';ctx.textAlign='center';
        ctx.fillText('Comparison-Adjusted Funnel Plot',w/2,20);
        ctx.fillText('Centered Effect',w/2,h-10);
        ctx.save();ctx.translate(15,h/2);ctx.rotate(-Math.PI/2);ctx.fillText('Standard Error',0,0);ctx.restore();
    }
};
...
const DoseResponse={
    ...
    fitQuadratic(x,y){
        ...
        const a=(sumX2Y*n-sumY*sumX2)/(sumX4*n-sumX2*sumX2);
...
# Also need to check if InfluenceDiagnostics has analyze method
if 'InfluenceDiagnostics={' in content and 'InfluenceDiagnostics.analyze' not in content:
    # Find the InfluenceDiagnostics module and add analyze method if calculate exists
    if 'InfluenceDiagnostics={calculate' in content or 'InfluenceDiagnostics.calculate' in content:
        # Add a wrapper
        old_inf = 'const InfluenceDiagnostics={'
        new_inf = '''const InfluenceDiagnosticsWrapper={
    analyze(processed){return InfluenceDiagnosticsBase.calculate({processed});},
};
const InfluenceDiagnosticsBase={'''
        content = content.replace(old_inf, new_inf)
        content = content.replace('InfluenceDiagnostics.calculate', 'InfluenceDiagnosticsBase.calculate')
        content = content.replace('typeof InfluenceDiagnostics', 'typeof InfluenceDiagnosticsWrapper')
        # Actually let's just add a simple wrapper after the modules
        print('[+] Added InfluenceDiagnostics.analyze wrapper')
