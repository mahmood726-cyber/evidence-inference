...
Only keep results with effect size AND confidence interval
"""
...
def main():
    print("=" * 70)
    print("  CLINICALTRIALS.GOV - EXTRACTING COMPLETE STATISTICAL RESULTS")
    print("=" * 70)
    print()
    sys.stdout.flush()
    ...
    next_token = None
    page = 0
    max_pages = 1000  # Up to 100,000 studies
    studies_checked = 0
    ...
    while page < max_pages:
        ...
        page += 1
        if page % 20 == 0:
            print(f"  Page {page}: {studies_checked:,} studies, {len(all_results):,} complete results")
            sys.stdout.flush()
